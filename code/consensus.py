...
def print_tables_with_results(results):
    ...
    print(type(results))
    df_print = pd.DataFrame(dict([ (k,pd.Series(v)) for k,v in results.items() ]))
    df_print.to_excel('df_print_results_.xlsx')
    with open('df_print_results_.txt','w') as d: 
        d.write(str(results))
    ...
    print(
        "\\textbf{Feature} & \\textbf{Mean fairness} & \\textbf{1} & \\textbf{2} & \\textbf{3} & \\textbf{1-3} & \\textbf{4} & \\textbf{5-7} & \\textbf{5} & \\textbf{6} & \\textbf{7} & \\textbf{7 point} & \\textbf{3 point} \\\\")
    print("\\hline")
    ...
    for i, feature in enumerate(sorted_features):
        print("{}, {}".format(i, ", ".join(
            [str(round(results[p][feature]["ternary_normalized_entropy"], 3)) for p in sorted_inherent_properties])))
